...
// Static variables
var CachedAliveMessage: IAliveMessage;		// Reuse ALIVE message object
...
export default class AppComponent
{
	...
	private joinHandle: NodeJS.Timer | null = null;
		...
		if (!usesAction) {
			...
			if (!usesAction) {
				for (const line of lines) {
					...
					if (maps) {
						maps = Array.isArray(maps) ? maps : [maps];
						if (maps.some(map => map.field === Constants.actionIdField)) {
							usesAction = true;
							break;
						}
					}
	...
	private processMessage(msg: IResponseMessage)
	{
		...
		switch (msg.$type) {
			...
			case "ControllersList": {
				...
				for (let id in Array.from(this.dataStore.keys())) {
			...
			case "ControllerStatus": {
				...
				if (!this.dataStore.has(id)) {
					...
					const ctrl = this.dataStore.get(id);
